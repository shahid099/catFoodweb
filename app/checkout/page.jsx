  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        localStorage.removeItem('cat_food_cart'); // Clear cart after successful placement
      } else {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-amber-900 hover:bg-amber-800 disabled:opacity-50 text-white font-bold py-3.5 px-4 rounded-xl shadow-md transition-colors text-sm flex items-center justify-center gap-2"
          >
